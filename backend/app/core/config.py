...
class Settings(BaseSettings):
    ...
    GROQ_API_KEY: str = Field(
        default="", description="API key for Groq Cloud inference"
    )
    GROQ_MODEL: str = Field(
        default="qwen/qwen3.8-27b", description="Model name for Groq chat completions"
    )
    ...
    JINA_EMBEDDING_MODEL: str = Field(
        default="jina-embeddings-v3", description="Jina embeddings model"
    )
    ...
    RAG_CHUNK_SIZE: int = Field(
        default=800, description="Chunk character size for document splitting"
    )
    RAG_CHUNK_OVERLAP: int = Field(
        default=150, description="Chunk overlap character count"
    )
    MAX_UPLOAD_SIZE_BYTES: int = Field(
        default=10 * 1024 * 1024, description="Max upload file size (10MB)"
    )

    # Observability & Monitoring
    SENTRY_DSN: str = Field(default="", description="Sentry DSN for error monitoring")
    OPIK_API_KEY: str = Field(
        default="", description="Comet Opik API key for LLM tracing"
    )
    OPIK_WORKSPACE: str = Field(default="default", description="Opik workspace name")
    OPIK_PROJECT_NAME: str = Field(
        default="eve-assistant", description="Opik project name"
    )
    AWS_REGION: str = Field(
        default="us-east-1", description="AWS Region for CloudWatch and cloud resources"
    )
